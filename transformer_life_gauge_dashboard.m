...
% transformer_life_gauge_dashboard in the Command Window).
    ...
    function tick()
        ...
        updateGauge(gLife, lifeRemaining, lifeColor, lifeStatus, '%.3f');

        % ---- Hot-spot gauge (thresholds match README: rated+15 / rated+25) ----
        if thetaH > C.critHS
        ...
        updateGauge(gHS, thetaH, hsColor, hsStatus, '%.1f');
...
function h = buildGauge(parent, pos, vmin, vmax, zones, unitStr)
    ...
    % colored zone arcs (zones is an Nx5 numeric matrix: [zmin zmax R G B])
    for i = 1:size(zones,1)
        zmin = zones(i,1); zmax = zones(i,2); col = zones(i,3:5);
        a1 = angleForValue(zmin, vmin, vmax);
        a2 = angleForValue(zmax, vmin, vmax);
        tt = linspace(min(a1,a2), max(a1,a2), 100);
        plot(ax, 1.0*cosd(tt), 1.0*sind(tt), 'LineWidth', 20, 'Color', col);
    end

    % tick labels at min / mid / max
    for tv = [vmin, (vmin+vmax)/2, vmax]
        ta = angleForValue(tv, vmin, vmax);
        text(ax, 1.22*cosd(ta), 1.22*sind(ta), sprintf('%g',tv), ...
             'Color', textMuted, 'FontSize', 9, 'HorizontalAlignment','center');
    end

    hNeedle = plot(ax,[0 0],[0 0.75],'LineWidth',3.5,'Color',[0.92 0.95 0.98]);
    plot(ax,0,0,'o','MarkerSize',11,'MarkerFaceColor',[0.92 0.95 0.98],'MarkerEdgeColor','none');
    hValText = text(ax,0,-0.30,'--','FontSize',30,'FontWeight','bold', ...
                     'HorizontalAlignment','center','Color',[1 1 1]);
    hUnitText = text(ax,0,-0.62,unitStr,'FontSize',10,'HorizontalAlignment','center','Color',textMuted); %#ok<NASGU>
    hStatusText = text(ax,0,-0.90,'','FontSize',12,'FontWeight','bold','HorizontalAlignment','center');

    h = struct('ax',ax,'needle',hNeedle,'valText',hValText,'statusText',hStatusText, ...
                'vmin',vmin,'vmax',vmax);
end
...
function updateGauge(h, value, statusColor, statusStr, fmt)
    ...
    r = 0.75;
    set(h.needle,'XData',[0 r*cosd(theta)],'YData',[0 r*sind(theta)],'Color',statusColor);
...
function a = angleForValue(v, vmin, vmax)
    ...
    a = 210 - frac*240;   % sweeps from 210 deg (min) clockwise through 90 deg to -30 deg (max)
end
